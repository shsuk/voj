...
<c:choose><c:when test="${code:codeInfo('root', item.key).edit_type=='month'}">
...
</c:when><c:when test="${item.key=='label'}">
	<label class="ui-dialog-title" style="color:#222222; font-size: 12px;line-height: 27px;">${item.value}</label>
...
</c:when><c:when test="${code:codeInfo('root', item.key).edit_type=='query_id'}">
	<tp:select_query name="${item.key}" qyeryId="${code:ref('code_mapping',item.key)}" selected="${row[attr.name]}" valid="[${valid}]" emptyText="${code:lang(item.key, lang, '') }을(를) 선택하세요." style="width:${ui.singleRow ? '200px' : '100px' };" />
	...
	<c:choose><c:when test="${cInfo.edit_type=='check'}">
		...
		<tp:radio name="${item.key}" groupId="${item.key}" checked="${param[item.key]}" valid="[${valid}]"/>
	</c:when><c:otherwise>
		<tp:select name="${item.key}" groupId="${item.key}" selected="${param[item.key]}" emptyText="${code:lang(item.key, lang, '') } 선택" valid="[${valid}]"  style="style=\"width:100px;height: 22px;margin: 0;\""/>
	</c:otherwise></c:choose>
</c:when><c:otherwise>
		<input type="text" style="width: 100px;" name="${item.key}" value="${req[item.key]}" ${item.value} ${fn:replace(item.value,'placeholder','title')} ${fn:replace(item.value,'placeholder','alt')} >
</c:otherwise></c:choose>