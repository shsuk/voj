...
<c:choose >
	<c:when test="${req.action=='u' }">
		<uf:organism>[
			<job:db id="count" query="voj/usr/user_status_update"/>
		]</uf:organism>
		<uf:organism test="${count==1 && !empty(param.email) && req.user_status!='Z' }" noException="true">[
			<job:mail id="send" to="${param.email }" template="${req.user_status=='Y' ? 'appl_user' : 'rej_user'  }" />
		]</uf:organism>
	...
	<c:when test="${req.action=='d' }">
		<uf:organism>[
			<job:db id="count" query="voj/usr/user_delete"/>
		]</uf:organism>
...
<script type="text/javascript">
	$(function() {
		...
		$('#del_btn').button({icons: {primary: "ui-icon-disk"}}).click(function(){
			$('#action').val('d');
			form_submit();
		});
	...
	<form id="main_pop_form" name="main_form" action="" onsubmit="return form_submit()">
		...
		<table class="${tpl_class_table}" >			
			...
			<tr>
				...
				<td>
					<input type="hidden" id="user_id"  name="user_id"  value="${row['USER_ID@dec'] }">
					${row['USER_ID@dec'] }
				</td>
			</tr>
			<tr>
			...
			<tr>
				<th class="${tpl_class_table_header}" title="user_nm">회원상태</th>
				<td>
					<tp:radio name="user_status" groupId="user_status" checked="${row.user_status}"/> 
				</td>
			</tr>
		</table>
		</div>
	</form>
	<c:if test="${row.user_status!='Y'}">
		<div id="del_btn" class="btn-ㅣ">삭제</div>
	</c:if>
