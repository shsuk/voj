...
import net.ion.webapp.controller.tag.UiTag;
...
{
	protected static final Logger logger = Logger.getLogger(CharsetMapper.class);
	...
	public static String getDefaultCharset(Locale locale) {
		if (locale == null) {
			logger.debug(CharsetMapper.class.getName() + ": null locale.");
			return "ISO-8859-1";
		} else {
	...
	/**
	 * locale의 default charset을 리턴한다. 참고) localeString은 다음과 같다. ko_KR,ja_JP,en_US... 또는 ko,ja,en,zh...
	 * 
	...
	public static String getDefaultCharset(String localeString) {
		...
		try {
			...
			logger.debug(CharsetMapper.class.getName() + ": unknown the default charset for language:" + localeString);
			charset = "ISO-8859-1";
		}
