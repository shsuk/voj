...
//		props.put("mail.smtp.starttls.enable", "true");
		...
		message = new UserMimeMessage(session, "<" + System.currentTimeMillis() + "." + from + ">");
	...
	public void sendWithSendMail(String from, String[] to, String[] cc, String[] bcc, String charset, String subject, String text, File[] attachments, String contentType) throws MailException {
		SendMail sm = new SendMail(smtpHost, smtpPort, smtpUser, smtpPass, null);
		sm.send(from, to, cc, bcc, charset, contentType, subject, text, attachments);
		sm = null;
	}
...
class SendingRunner implements Runnable {
	...
	protected SendingRunner(SendMail owner) {
		this.owner = owner;
		this.log = LoggerFactory.getLogger(SendMail.class);
	}
		...
		for (;;) {
			try {
				...
				synchronized (owner.waitLine) {
					if (owner.waitLine.isEmpty()) break;
				...
				if (owner.exceptionReceiver != null) owner.exceptionReceiver.receive(me);
				log.warn(me.getMessage());
			} catch (Throwable t) {
				if (owner.exceptionReceiver != null) owner.exceptionReceiver.receive(new MailException("unexpected error", t));
